...
class PoolsScreenerTask(BaseTask):
    ...
    def __init__(self, config):
        ...
        self.mongo_config = self.config.config.get("mongo_config", {})
        self.local_output_dir = Path(
            self.config.config.get(
                "output_dir",
                data_paths.base_path / "app" / "outputs" / "screeners" / "pools",
            )
        )
        ...
        try:
            ...
            if self.mongodb_client is None and self.mongo_config.get("uri"):
                try:
                    ...
                    logging.warning(f"Unable to initialize task-level MongoDB client: {mongo_exc}")
    ...
    def clean_pools(self, pools: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            return pools[pools["quote"] == self.quote_asset]
        except Exception as e:
    ...
    async def execute(self, context: TaskContext) -> Dict[str, Any]:
        ...
        try:
            ...
            output_file = await self._store_document(document)
    ...
    async def _store_document(self, document: Dict[str, Any]) -> Path:
        self.local_output_dir.mkdir(parents=True, exist_ok=True)
        output_file = self.local_output_dir / f"{self.network}_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
        ...
        if self.mongodb_client is not None:
            try:
                await self.mongodb_client.insert_documents(
                    collection_name=self.mongo_config.get("collection", "pools"),
                    documents=[document],
                    db_name=self.mongo_config.get("db"),
                )
            except Exception as exc:
                logging.warning(f"MongoDB storage failed for pools screener results: {exc}")
